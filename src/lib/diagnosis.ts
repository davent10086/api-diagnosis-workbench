...
async function completion(model: string, messages: unknown[], responseFormat = true) {
...
async function extractImage(asset: { id: string; filePath: string; extraction: unknown }) {
  ...
  try {
    ...
    const content = await completion(process.env.VISION_MODEL || "qwen-vl-max", [{ role: "system", content: "你是 API 排障证据提取器。只识别截图中可见的错误码、时间、请求 ID、Trace ID、接口、状态码和日志。不要猜测被遮蔽或不可见内容。输出 JSON：{summary:string,fields:string[]}。" }, { role: "user", content: [{ type: "text", text: "提取这张已脱敏截图中的排障事实。" }, { type: "image_url", image_url: { url: `data:${mime};base64,${data.toString("base64")}` } }] }]);
...
export async function runDiagnosis(caseId: string) {
  ...
  try {
    ...
    for (const asset of approvedImages) images.push(await extractImage(asset));
    ...
    const prompt = { trace: traceInput, rules: findings, imageEvidence: images, knowledge: knowledge.slice(0, 5).map((x) => ({ title: x.title, url: x.sourceUrl, excerpt: x.body.slice(0, 1200) })), similarCases: similar, instructions: "仅基于提供的证据诊断 API 故障。confirmed_evidence 每项必须明确引用 rule:、trace:、image: 或 knowledge: 前缀；无直接证据的结论放入 hypotheses。严格输出 JSON，字段为 summary, root_cause, confidence, severity, fault_layer, confirmed_evidence, hypotheses, missing_evidence, next_actions, customer_message。" };
    const raw = await completion(process.env.ANALYSIS_MODEL || "qwen-plus", [{ role: "system", content: "你是审慎的 API 排障工程师。不得把假设写成已确认根因。" }, { role: "user", content: JSON.stringify(prompt) }]);
    ...
    await db.transaction(async (tx) => {
      await tx.update(diagnosisRuns).set({ report: { state: "failed", error: message }, durationMs: Date.now() - started, status: "failed" }).where(eq(diagnosisRuns.id, run[0].id));
