...
export type Report={symptom:string;severity:Severity;fault_layer:Layer;confidence:number;evidence:string[];official_evidence:{title:string;vendor:string;url:string;excerpt:string}[];possible_causes:string[];missing_evidence:string[];next_checks:string[];external_message:string};
