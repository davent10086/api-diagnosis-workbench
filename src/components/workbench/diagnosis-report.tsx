    ...
    <section className="panel p-4">
      ...
      {findings.length === 0 ? (
        ...
        findings.map((finding) => (
          <p
            className="mt-3 border-l-2 border-amber-500 bg-amber-50 px-3 py-2 text-sm leading-5"
            key={finding.ruleId}
          >
    ...
    <aside className="panel h-fit overflow-hidden p-0">
      ...
      <div className="border-l-2 border-amber-500 bg-amber-50 px-3 py-3 text-sm">
        ...
        <p className="mt-2 text-xs font-medium text-orange-700">
          置信度 {report.confidence}% · {report.fault_layer}
        </p>
      </div>
      <h3 className="mt-5 text-sm font-semibold">下一步检查</h3>
      {report.next_checks.map((item) => (
        <p className="mt-2 border-b border-slate-100 pb-2 text-sm text-slate-700" key={item}>
          {item}
        </p>
      ))}
